...
case class PreprocessTableMerge(override val conf: SQLConf)
  ...
  def apply(mergeInto: DeltaMergeInto, transformToCommand: Boolean): LogicalPlan = {
    ...
    additionalColumns = IdentityColumn.getIdentityColumns(
   ...
   *        resolved.
   * We also return columns names for which we will high water marks.
   */
  private def resolveImplicitColumns(
    ...
    // Column names for which we will track IDENTITY high water marks.
