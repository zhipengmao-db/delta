...
trait IdentityColumnConflictSuiteBase
  ...
  // TODO: Make a version of this that doesn't use row tracking for OSS.
...
//      DeltaConfigs.ROW_TRACKING_ENABLED.key -> "false",
  ...
  /**
   * We are testing the following combinations (see [[ConflictCheckerEdge.checkNoMetadataUpdates]]
   * for details).
   *
  ...
  test("high watermark changes after analysis but before execution of merge") {
    val tableName = "table"
    withIdentityColumnTable(GeneratedAsIdentityType.GeneratedAlways, tableName) {
      // Create a QueryExecution object for a MERGE statement, and it force it to be analyzed,
      ...
      sql(s"INSERT INTO $tableName (value) VALUES (0)")
