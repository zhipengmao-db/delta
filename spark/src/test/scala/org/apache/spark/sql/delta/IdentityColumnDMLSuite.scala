...
trait IdentityColumnDMLSuiteBase
  ...
  // different variations - enable/disable LSM, enable/disable WSCG, vary num partitions in the
  // identity column generation stage.
  ...
  test("CDF for tables with identity column - MERGE") {
    ...
    withTable(tgt) {
      ...
      withTempView(src) {
        (1 :: 20 :: 30 :: Nil).toDF("val").createOrReplaceTempView(src)
        sql(
          s"""MERGE INTO $tgt target
             |USING $src src ON target.val = src.val
      ...
      checkAnswer(sql(
        s"""SELECT key, val, ${CDCReader.CDC_TYPE_COLUMN_NAME}
           ...
           |ORDER BY key, val, _change_type""".stripMargin),
        Seq(
  ...
  test("CDF for tables with identity column - UPDATE") {
    ...
    withTable(tgt) {
      ...
      sql(
        s"""UPDATE $tgt
           |SET val = val + 100
           |WHERE key < 3""".stripMargin)

      checkAnswer(sql(
        s"""SELECT *
           |FROM $tgt
           |ORDER BY key, val""".stripMargin),
        Seq(
