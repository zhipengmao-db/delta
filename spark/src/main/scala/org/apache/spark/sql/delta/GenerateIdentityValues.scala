...
import org.apache.spark.sql.catalyst.expressions.{RuntimeReplaceable, UnaryExpression}
import org.apache.spark.sql.catalyst.expressions.codegen.{CodegenContext, CodeGenerator, ExprCode, FalseLiteral}
import org.apache.spark.sql.catalyst.expressions.codegen.Block._
import org.apache.spark.sql.types.{DataType, LongType}

/**
 * This expression is a wrapper around the GenerateIdentityValues expression, used to
 * pass the UDF allow-list validation CheckAnalysis stage. It is replaced with the
 * GenerateIdentityValues expression during the finish Analysis phase.
 *
 * GenerateIdentityValues is a special UDF that we can allow list specifically for its use
 * in Delta MERGE. We don't want to allow list all UDFs in MERGE nor change the
 * CheckAnalysis validation, as both alternatives have much broader impact and is beyond
 * the scope of enabling Identity Columns support in Delta.
 */
case class GenerateIdentityPlaceholderExpr(
    generateIdentityValues: GenerateIdentityValues)
  extends UnaryExpression with RuntimeReplaceable {
  override def replacement: Expression = generateIdentityValues

  // We force this expression to be deterministic to pass the CheckAnalysis rule
  // for deterministic expressions, as a means to allow list GenerateIdentityValues.
  override lazy val deterministic: Boolean = true

  override def child: Expression = generateIdentityValues
  override protected def withNewChildInternal(newChild: Expression):
  GenerateIdentityPlaceholderExpr = {
    copy(generateIdentityValues = newChild.asInstanceOf[GenerateIdentityValues])
  }
}
