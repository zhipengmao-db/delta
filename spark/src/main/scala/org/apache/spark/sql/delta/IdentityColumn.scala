...
object IdentityColumn extends DeltaLogging {
  ...
  def getNumberOfIdentityColumns(schema: StructType): Int = {
  ...
  def createIdentityColumnStatsTracker(
      ...
      optimizerStatsEnabled: Boolean) : Option[DeltaIdentityColumnStatsTracker] = {
    ...
    Some(new DeltaIdentityColumnStatsTracker(hadoopConf, path, statsDataSchema, resolvedExpr,
      identityColumnInfo, optimizerStatsEnabled))
  }
