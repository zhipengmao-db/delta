...
class DeltaCatalog extends DelegatingCatalogExtension
  ...
  // During parsing we store the generation expression at both a delta-specific key in addition
  // to the key used in OSS Spark. When the data source is not Delta we need to remove this key
  // from the column metadata
  // TODO: When the data source is not Delta we need to use the V2 column APIs to propagate any
  //   generated columns (see SPARK-42398 and SPARK-41290). This is okay for now since we only
  //   expect generated columns to be supported for Delta.
  private def removeDeltaGeneratedColumnMetadata(schema: StructType): StructType = {
    ColumnWithDefaultExprUtils.removeDefaultExpressions(schema, keepIdentityColumns = true)
  }
    ...
    recordFrameProfile("DeltaCatalog", "createTable") {
      if (DeltaSourceUtils.isDeltaDataSourceName(getProvider(properties))) {
        ...
        return createCatalogTable(
          ident,
          removeDeltaGeneratedColumnMetadata(schema),
          partitions,
          properties
        )
      }
