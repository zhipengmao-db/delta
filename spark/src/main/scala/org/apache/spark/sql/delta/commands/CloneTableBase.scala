  ...
  with SQLConfHelper
{
  ...
  private def prepareSourceMetadata(
    ...
    // TODO(LC-1355): IdentityColumn is not fully OSSed so we don't need to have this
    // in OSS at the moment.
    val clonedSchema =
    ...
    clonedMetadata = clonedMetadata.copy(schemaString = clonedSchema.json)
