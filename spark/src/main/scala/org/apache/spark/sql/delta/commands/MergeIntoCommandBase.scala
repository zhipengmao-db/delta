      ...
      deltaTxn.metadata.schema.zip(clause.resolvedActions.map(_.expr)).foreach {
        case (f, GenerateIdentityValues(gen)) =>
          ...
          if (info.highWaterMark != gen.highWaterMarkOpt) {
            ...
            // Clear the testing barriers to avoid deadlocks during fuzz / concurrency tests.
            deltaTxn.executionObserver.preparingCommit()
            deltaTxn.executionObserver.beginDoCommit()
            deltaTxn.executionObserver.beginBackfill()
            deltaTxn.executionObserver.transactionAborted()
